...
DB_PASSWORD = os.getenv("DB_PASSWORD", "Frizon12!")
...
class Ligacao(db.Model):
    ...
    resultado = db.Column(db.Enum('comprou', 'nao_comprou', 'retornar', 'sem_interesse', 'relacionamento'),
                          default='nao_comprou')
...
def registrar_ligacao(cliente_id: int):
    ...
    try:
        ...
        if resultado not in ('comprou', 'nao_comprou', 'retornar', 'sem_interesse', 'relacionamento'):
...
with app.app_context():
    ...
    # garantir enum com 'relacionamento' em ligacoes.resultado
    try:
        db.session.execute(text(
            "ALTER TABLE ligacoes MODIFY COLUMN resultado "
            "ENUM('comprou','nao_comprou','retornar','sem_interesse','relacionamento') "
